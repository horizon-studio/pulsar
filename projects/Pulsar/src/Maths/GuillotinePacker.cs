...
namespace Pulsar.Maths
{
    ...
    public class GuillotinePacker
    {
        ...
        public bool TryInsert(int width, int height, int count, InsertRectangleCallback inserted)
        {
            ...
            foreach (var freeRectangle in freeRectangles) tempFreeRectangles.Add(freeRectangle);
            ...
            freeRectangles.Clear();
            foreach (var tempFreeRectangle in tempFreeRectangles) freeRectangles.Add(tempFreeRectangle);
            tempFreeRectangles.Clear();

            return true;
        }

        private static bool Insert(int width, int height, List<Rectangle> freeRectanglesList,
            ref Rectangle bestRectangle)
        {
            // Info on algorithm: http://clb.demon.fi/files/RectangleBinPack.pdf
            var bestScore = int.MaxValue;
            var freeRectangleIndex = -1;

            // Find space for new rectangle
            for (var i = 0; i < freeRectanglesList.Count; ++i)
            {
            ...
            var w = freeRectangle.Width - bestRectangle.Width;
            var h = freeRectangle.Height - bestRectangle.Height;
            var splitHorizontal = bestRectangle.Width * h > w * bestRectangle.Height;

            // Form the two new rectangles.
            var bottom = new Rectangle
            {
                X = freeRectangle.X, Y = freeRectangle.Y + bestRectangle.Height,
                Width = splitHorizontal ? freeRectangle.Width : bestRectangle.Width, Height = h
            };
            var right = new Rectangle
            {
                X = freeRectangle.X + bestRectangle.Width, Y = freeRectangle.Y, Width = w,
                Height = splitHorizontal ? bestRectangle.Height : freeRectangle.Height
            };
