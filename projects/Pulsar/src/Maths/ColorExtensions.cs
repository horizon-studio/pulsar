...
namespace Pulsar.Maths
{
    ...
    {
        /// <summary>
        ///     Indicates if the given string can be converted to an <see cref="uint" /> RGBA value using
        ///     <see cref="StringToRgba" />.
        /// </summary>
        /// <param name="stringColor">The string to convert.</param>
        /// <returns>True if the string can be converted, false otherwise.</returns>
        public static bool CanConvertStringToRgba([AllowNull] string stringColor)
        {
        ...
        public static uint StringToRgba([AllowNull] string stringColor)
        {
            var intValue = 0xFF000000;
            if (stringColor != null)
                if (stringColor.StartsWith("#"))
                {
                    if (stringColor.Length == "#000".Length && uint.TryParse(stringColor.Substring(1, 3),
                        NumberStyles.HexNumber, null, out intValue))
                        intValue = ((intValue & 0x00F) << 16)
                                   ...
                                   | ((intValue & 0xF00) >> 8)
                                   | 0xFF000000;
                    if (stringColor.Length == "#000000".Length && uint.TryParse(stringColor.Substring(1, 6),
                        NumberStyles.HexNumber, null, out intValue))
                        intValue = ((intValue & 0x000000FF) << 16)
                                   | (intValue & 0x0000FF00)
                                   | ((intValue & 0x00FF0000) >> 16)
                                   | 0xFF000000;
                    if (stringColor.Length == "#00000000".Length && uint.TryParse(stringColor.Substring(1, 8),
                        NumberStyles.HexNumber, null, out intValue))
                        intValue = ((intValue & 0x000000FF) << 16)
                                   | (intValue & 0x0000FF00)
                                   | ((intValue & 0x00FF0000) >> 16)
